...
public class CatchAgentV1 : Agent
{
    float currentTs = 0f;
    ...
    private List<float> ShootRaysTarget(){
        ...
        for (int i=0; i<raysToShoot; i++) {  
            ...
            angle += 2*Mathf.PI/3 / (raysToShoot-1);

            Vector3 dir = new Vector3(x, 0, z);
            RaycastHit hit;
            Debug.DrawLine(Target.transform.position, Target.transform.position + maxDist*dir, Color.red);
